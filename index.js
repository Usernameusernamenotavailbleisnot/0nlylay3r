...
const SwapManager = require('./src/swap');  // Import the SwapManager
...
async function loadConfig() {
    try {
        ...
        return {
            ...
            swap: {  // Default swap configuration
                enable_swap: true,
                swap_count: 3,
                swap_amount: {
                    min: 0.00001,
...
async function main() {
    while (true) {
        console.log(chalk.blue.bold('\n=== OnlyLayer Ethereum Testnet Automation Tool ===\n'));
        ...
        try {
            ...
            for (let i = 0; i < privateKeys.length; i++) {
                ...
                // 2. Execute swap operations if enabled (immediately after bridge)
                if (config.swap && config.swap.enable_swap) {
                    try {
                        console.log(chalk.blue.bold(`\n=== Running Swap Operations for Wallet ${walletNum} ===\n`));
                        
                        // Small delay before starting swaps
                        console.log(chalk.cyan(`${getTimestamp(walletNum)} ℹ Waiting 5 seconds before starting swaps...`));
                        await new Promise(resolve => setTimeout(resolve, 5000));
                        
                        // Initialize swap manager with wallet's private key and current config
                        const swapManager = new SwapManager(pk, config);
                        swapManager.setWalletNum(walletNum);
                        
                        // Execute swap operations
                        await swapManager.executeSwapOperations();
                        
                    } catch (error) {
                        console.log(chalk.red(`${getTimestamp(walletNum)} ✗ Error in swap operations: ${error.message}`));
                    }
                }

                // 3. Transfer tokens to self if enabled
